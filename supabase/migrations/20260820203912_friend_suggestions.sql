...
AS $function$
  ...
  -- Anyone already linked in ANY state. Pending counts: re-suggesting someone
  -- you have already asked is noise, and re-suggesting someone who asked you
  -- would duplicate the request sitting in /friends.
  connected as (
    select case when f.requester = v.id then f.addressee else f.requester end as id
    from public.friendships f
    cross join viewer v
    where v.id in (f.requester, f.addressee)
  ),
  my_friends as (
    select case when f.requester = v.id then f.addressee else f.requester end as id
    from public.friendships f
  ...
  cross join viewer v
  where cand.id <> v.id
    and not exists (select 1 from connected c where c.id = cand.id)
